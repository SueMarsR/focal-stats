...
// Apple system-font stack matching the web app. Multi-word family names are
// single-quoted: this string goes into a double-quoted SVG attribute
// (font-family="..."), and embedded double quotes would break the XML and the
// card would fail to load as an <img> for rasterization.
const SANS =
  "-apple-system,BlinkMacSystemFont,'SF Pro Display','SF Pro Text',system-ui,'PingFang SC','Microsoft YaHei',sans-serif";
...
export interface ShareCardOpts {
  ...
  url?: string;
  /** Match the web app's current appearance; defaults to light. */
  theme?: CardTheme;
}
...
function deviceText(label: string, x: number, y: number, p: CardPalette): string {
 ...
 */
function cameraLine(top: GroupStat, realCount: number, y: number, p: CardPalette): string {
...
function lensLine(top: GroupStat, realCount: number, y: number, p: CardPalette): string {
 ...
 * + lens, and project branding. Styled to match the web app's Apple light/dark look.
 * Pure and deterministic so it can be unit-tested; the caller rasterizes it to PNG.
 */
export function shareCardSvg(stats: FocalStats, opts: ShareCardOpts = {}): string {
  ...
  return [
    ...
    `<defs>`,
    `<linearGradient id="cardBg" x1="0" y1="0" x2="0.25" y2="1"><stop offset="0%" stop-color="${p.bgFrom}"/><stop offset="100%" stop-color="${p.bgTo}"/></linearGradient>`,
    `<linearGradient id="heroGrad" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="${p.gradFrom}"/><stop offset="100%" stop-color="${p.gradTo}"/></linearGradient>`,
    `</defs>`,
    `<rect width="${W}" height="${H}" fill="url(#cardBg)"/>`,
    `<rect x="16" y="16" width="${W - 32}" height="${H - 32}" rx="28" fill="none" stroke="${p.border}" stroke-width="1.5"/>`,

    // Header
    `<text x="${PAD}" y="100" font-size="26" letter-spacing="6" fill="${p.muted}" font-family="${SANS}" font-weight="600">FOCAL STATS · 焦段统计</text>`,
    `<rect x="${PAD}" y="120" width="64" height="4" rx="2" fill="${p.accent}"/>`,

    // Hero — number filled with the system-blue → indigo gradient
    `<text class="hero-num" x="${PAD}" y="310" font-size="190" fill="url(#heroGrad)" font-family="${SANS}" font-weight="700">${heroFocal}<tspan font-size="64" fill="${p.muted}" dx="8" font-weight="500">mm</tspan></text>`,
    ...
    // Footer
    `<rect x="${PAD}" y="${footerRuleY}" width="${W - PAD * 2}" height="1" fill="${p.border}"/>`,
    `<text x="${PAD}" y="${footerTextY}" font-size="26" fill="${p.muted}" font-family="${SANS}">${escHtml(url)}</text>`,
